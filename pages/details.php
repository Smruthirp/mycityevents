...
if ($mail->send())
	...
	$msg = "Message not sent, please try again!";
		...
		echo' 
		<section>
		<img class="image" src="data:image/jpeg;base64,'.base64_encode($row['myimage']).'"/>
		
		<div class="text">
			<table style="width:100%">
				<tr>
					<th colspan="3">'.$row['event'].' <div style="text-transform: lowercase; display: inline-block; width: auto;"> in </div> '.$city.'</th>
				</tr>
				
				<tr>
					<td><button>From</button> <i class="fa fa-calendar"></i>'.$stDate.'</td>
					<td><button>To</button> <i class="fa fa-calendar"></i>'.$enDate.'</td>
					<td>
						<span class="dropdown">
						<button class="dropbtn">ADD TO CALENDER</button>
						<span class="dropdown-content">
							<a target="_blank" 
								href="https://calendar.google.com/calendar/r/eventedit?text='.urlencode($row['event']).'&dates='.date("Ymd", strtotime($sDate)).'T'.date("His", strtotime($sTime)).'/'.date("Ymd", strtotime($eDate)).'T'.date("His", strtotime($eTime)).'&details=For+details,+link+here:+'."http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]".'&location='.urlencode($row['location']).'">Add to Google calender</a>
							<a target="_blank" 
								href="ical.php?start='.date("Ymd", strtotime($sDate)).'T'.date("His", strtotime($sTime)).'&end='.date("Ymd", strtotime($eDate)).'T'.date("His", strtotime($eTime)).'&summary='.urlencode($row['event']) .'&discription=For+details,+link+here:+'."http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]".'">Add to Apple calendar</a>
							<a target="_blank" 
						  		href="ical.php?start='.date("Ymd", strtotime($sDate)).'T'.date("His", strtotime($sTime)).'&end='.date("Ymd", strtotime($eDate)).'T'.date("His", strtotime($eTime)).'&summary='.urlencode($row['event']) .'&discription=For+details,+link+here:+'."http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]".'">Add to Outlook</a>
						</span>
			  			</span>
			  		</td>					
				</tr>
				...
				<tr>
					<td><button>At</button> <i class="fa fa-clock-o"></i>'.$stTime.'</td>
					<td><button>/</button> <i class="fa fa-clock-o"></i>'.$enTime.'</td>					
				</tr>

				<tr>
					<td colspan="2"><button>Venue</button> <i class="fa fa-map-marker"></i> '.$row['location'].'</td>
					<td><a target="_blank" href="https://www.google.co.in/maps?q='.$row['location'].'" class="view_map">VIEW IN GOOGLE MAP</a></td>
				</tr>

				<tr>
					<td colspan="3"><button>Created by</button> <i class="fa fa-user"></i>'.$row['user'].'</td>
				</tr>
